...
Let's take a historical [Storting melding](data/stmeld8_kongo.pdf). The front page looks like this.
...
The way OCR works is that the algorithms are made to "learn" by ingesting a large amount of data for which the right answers are known (ie. images of text that have been, perfectly, digitized). The algorithms trained to recognize English are included in the package, anything else has to be downloaded by hand as shown below (to find you needed language's three letter code, see the [https://github.com/tesseract-ocr/tessdata](tessdata) github repository and especially the list of available languages [https://tesseract-ocr.github.io/tessdoc/Data-Files-in-different-versions.html](here)). 
